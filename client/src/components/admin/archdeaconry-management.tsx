...
import { insertArchdeaconrySchema, type Archdeaconry, type InsertArchdeaconry, type Parish } from "@shared/schema";
import { Plus, Edit, Trash2, MapPin, Church } from "lucide-react";

export default function ArchdeaconryManagement() {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingArchdeaconry, setEditingArchdeaconry] = useState<Archdeaconry | null>(null);
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<InsertArchdeaconry> }) => {
      const res = await apiRequest("PUT", `/api/admin/archdeaconries/${id}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/archdeaconries"] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Archdeaconry Management</h3>
          <p className="text-gray-600">Manage archdeaconries and their regions</p>
        </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                {editingArchdeaconry ? "Edit Archdeaconry" : "Add New Archdeaconry"}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <Label htmlFor="image1Url" className="text-sm">Image 1 URL</Label>
                  <Input
                    id="image1Url"
                ...
                <div>
                  <Label htmlFor="image2Url" className="text-sm">Image 2 URL</Label>
                  <Input
                    id="image2Url"
                ...
                <div>
                  <Label htmlFor="image3Url" className="text-sm">Image 3 URL</Label>
                  <Input
                    id="image3Url"
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No archdeaconries yet</h3>
            <p className="text-gray-600">Get started by adding your first archdeaconry.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {archdeaconries.map((archdeaconry) => (
            ...
            >
              ...
              <CardContent>
                ...
                {/* Image Gallery */}
                {(archdeaconry.image1Url || archdeaconry.image2Url || archdeaconry.image3Url) && (
